      ...
      <div class="row">
		...
		<section class="col-lg-12 connectedSortable">
          <!-- TO DO List -->          
            ...
            <div class="box-body">			<?php         $attributes = ['name' => 'formEdit', 'id' => 'formEdit'];        		echo form_open_multipart('', $attributes);      ?>              <!--<form method="post" enctype="multipart/form-data">-->		   			   <?php 			   if(form_error('body'))			   {				echo form_error('body');   			   }			   ?>			   Download Category <span style="color:red;">*</span>                <div class="form-group">                  <select class="form-control" name="download_category">				  <option value="">Select</option>				  <option value="Book" <?php if($getdownload->download_category=='Book') echo"selected";?>>Book</option>				  <option value="Forms" <?php if($getdownload->download_category=='Forms') echo"selected";?>>Forms</option>				  </select>				  <div style="color:red"><?php echo form_error('download_category');?></div>                </div>				
			  Download Title <span style="color:red;">*</span>
                <div class="form-group">
                  <input type="text" class="form-control" name="download_title" value="<?php echo str_replace("^","'",$getdownload->download_title); ?>" placeholder="Download Title">                  <div style="color:red"><?php echo form_error('download_title');?></div>
                </div>											
				Status <span style="color:red;"></span>
				<div class="radio">
                    <label>
					  <span><input type="radio" name="pg_status" value="Active" <?php if($getdownload->pg_status == 'Active') echo "checked"; ?>/></span>
<span>Active</span>  <span style="margin-left:30px;">
<input type="radio" name="pg_status"  value="Inactive" <?php if($getdownload->pg_status == 'Inactive') echo "checked"; ?> /></span><span>Inactive</span>
                    </label>
                  </div>
				
				<div class="form-group">
				<label for="exampleInputFile">File <span style="color:red;">*</span></label>
                <!--<input type="file" name="image" id="exampleInputFile">-->                <?php				echo form_upload(['name'=>'file']);				?>				<a href="<?php echo base_url().'upload/download/'.$getdownload->download_file; ?>">DOWNLOAD</a>
  ...
  <!-- /.content-wrapper -->
<?php include("footer.php");?><script>$('#summernote').summernote("code",`<?php echo str_replace("^","'",$getdownload->description); ?>`);</script>
