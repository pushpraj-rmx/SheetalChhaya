        ...
        $("#page-msg").fadeOut(2000)
           ...
           <?php if($this->session->flashdata('sucmsg')): ?>    <p><?php echo $this->session->flashdata('sucmsg'); ?></p><?php endif; ?></div>
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>		        <?php		        $pi=1;				$totecpg=count($blogcategorycontent);				if(basename($_SERVER['REQUEST_URI'])>1)				{				$pg=basename($_SERVER['REQUEST_URI']);					}				else{				$pg=10;					}				$pgnum=$pg/10;				if(count($blogcategorycontent)):				foreach($blogcategorycontent as $contents):				if($pgnum>0)				{				$pg=$pgnum*10;				$sn=$pg+$pi;					}				else				{				$sn=$pi;					}                ?>
                <tr>
				 <td><?php echo $sn;?></td>
                  <td><?=str_replace("^","'",$contents['blog_category']);?></td>
                 <td><?=$contents['pg_url'];?></td>
                 <td>                                  <?php if ($contents['pg_status'] == 'Active') : ?>									<a href="<?php echo base_url('sitepanel/') ?>blogcategorypgstatus/<?=$contents['blog_cat_id'];?>/Inactive"><span class="label label-info">Active</span></a>                                    <?php else: ?>                                     <a href="<?php echo base_url('sitepanel/') ?>blogcategorypgstatus/<?=$contents['blog_cat_id'];?>/Active"><span class="label bg-red">Inactive</span></a>                                    <?php endif ?>									</td>
