        ...
        <div class="col-xs-12">
          ...
          <div class="box">
            <div class="box-header">
             <button type="button" class="btn bg-navy margin" onclick="window.location.href='<?php echo base_url('sitepanel/addfaq') ?>'">Add Faq's</button>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>		        <?php		        $pi=1;				$totecpg=count($faqcontent);				if(basename($_SERVER['REQUEST_URI'])>1)				{				$pg=basename($_SERVER['REQUEST_URI']);					}				else{				$pg=10;					}				$pgnum=$pg/10;				if(count($faqcontent)):				foreach($faqcontent as $contents):				if($pgnum>0)				{				$pg=$pgnum*10;				$sn=$pg+$pi;					}				else				{				$sn=$pi;					}                ?>
                <tr>
				 <td><?php echo $sn;?></td>
                  <td><?=$contents['pg_name'];?></td>
                 <td><?=$contents['pg_url'];?></td>
                 <td>                                  <?php if ($contents['pg_status'] == 'Active') : ?>									<a href="<?php echo base_url('sitepanel/') ?>faqstatus/<?=$contents['pg_id'];?>/Inactive"><span class="label label-info">Active</span></a>                                    <?php else: ?>                                     <a href="<?php echo base_url('sitepanel/') ?>faqstatus/<?=$contents['pg_id'];?>/Active"><span class="label bg-red">Inactive</span></a>                                    <?php endif ?>									</td>
				  
                  <td><a href="<?php echo base_url('sitepanel/') ?>faqedit/<?=$contents['pg_id'];?>">
